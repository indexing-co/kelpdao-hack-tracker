    ...
    <>
      <div className="mb-6">
        <p className="text-ink-300 text-sm leading-relaxed">
          ...
          Reads directly from{' '}
          <code className="font-mono text-xs text-ink-100">EndpointV2.getConfig()</code>
          . Last run: <span className="num-tabular text-ink-100">{lastRun}</span>.
        </p>
        <p className="text-ink-500 text-xs mt-2 leading-relaxed">
          Why this exists: KelpDAO was drained $292M because their bridge ran 1-of-1 DVN.
          CoinDesk reported ~40% of LayerZero apps still ran the same config. This tab tracks
          who's still exposed.
        </p>
        <p className="text-ink-500 text-xs mt-2 leading-relaxed">
          Note on data: these are <em>current state</em> snapshots from{' '}
          <code className="font-mono text-xs text-ink-300">eth_call</code>, not historical
          config drift. The daily cron builds a time series so changes become visible going
          forward. Event-driven backfill (indexing{' '}
          <code className="font-mono text-xs text-ink-300">UlnConfigSet</code> on EndpointV2)
          is the next step for true historical coverage.
        </p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        <CensusCounter label="OApps tracked" value={stats.total_oapps} />
        <CensusCounter label="Routes verified" value={stats.total_routes} />
        <CensusCounter
          ...
          sub={
            stats.total_routes > 0
              ? `${Math.round((stats.routes_at_1of1 / stats.total_routes) * 100)}% of total`
              : undefined
          }
        />
        <CensusCounter
          ...
          sub={
            stats.total_routes > 0
              ? `${Math.round((stats.routes_4plus / stats.total_routes) * 100)}% of total`
              : undefined
          }
        />
      </div>
      ...
      {oneOfOneRoutes.length > 0 && (
        <section className="mb-8 border border-accent-warn/40 rounded-card bg-accent-warn/5 p-6">
          <h2 className="text-lg font-semibold text-accent-warn mb-2">
            ⚠ OApps still running 1-of-1 DVN ({oneOfOneRoutes.length}{' '}
            {oneOfOneRoutes.length === 1 ? 'route' : 'routes'})
          </h2>
          ...
          <CensusTable rows={oneOfOneRoutes} />
        </section>
      )}
...
function CensusTable({ rows }: { rows: OAppDvnConfig[] }) {
  if (rows.length === 0) {
    return (
